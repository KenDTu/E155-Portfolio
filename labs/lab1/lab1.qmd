...
In this lab, the E155 Microprocessor PCB was soldered with the UPduino v3.1 FPGA, an STM  Nucleo-L432KC MCU board, and various switches, SMT resistors, LEDS, and other electronic components. A design was implemented on the FPGA to demonstrate the functionality of the on-board high-speed oscillator by blinking one of the on-board LEDs, a simple XOR and AND combinational logic to light up two other on-board LEDs, and logic control signal from a 4 bit representation DIP switch to toggle an off-board seven segment LED to represent a single hexademical digit. The high speed oscillator was configured at a frequency of 24 MHz and divided down using a counter to achieve a blinking frequency of ~2.4 Hz. 

## Design and Testing Methodology

The on-board high-speed oscillator (HSOSC) from the iCE40 UltraPlus primitive library was used to generate a clock signal at 24 MHz. Then, a counter was used to divide the high frequency clock signal down so that the blinking frequency could be easily visualized using one of the on-board LEDs. Counter is a variable with a width of 24 bits. Therefore when selecting the bit to represent the LED we can set the 2.4Hz blinking LED to be the most significant 24th bit as this will be on. The $$\text{target frequency = input frequency / division factor} $$ Therefore as the target frequency is 2.4Hz and the input frequency is 24MHz, we require the division factor to close to 10,000,000. We can roughly achieve this by selecting the 24th bit to be a 1 because $$2^{23} = 8,388,608 \implies \text{target frequency} = \frac{2.4MHz}{8,388,608} \approx 2.4Hz$$
...
![Figure 1: Block diagram for the synthesis of the top module](images/blockdiagram.jpeg)
:::

<!-- ::: {.callout-note collapse="true"}
## Figure X: Image title

![image caption](path/to/image.jpeg){#fig-blockdiagram}
...
![Figure 2: Schematic diagram for the physical layout](images/schematic.jpeg)
:::
...
![Figure 3: Seven segment LED displaying 8 as hexademical representation when the DIP switch is turned on](images/sevenseg.jpeg)
:::
...
![Figure 4: QuestSim simulation showing the internal signals and passing LED test cases for the one line assign statements and the seven segment](images/simulation.png)
:::
...
The design successfully produces SystemVerilog files and project synthesis to verify logic controls to two LEDs, a blinking LED with a clock divider, and a seven segment LED to represent the hexadecimal representation of 8 bit signal from a DIP switch. In total I spent 50 hours on this lab in one week.
...
Figures 5 and 6 demonstrate the output when the SystemVerilog code was synthesized. 

::: {.callout-note collapse="true"}
## Figure 5: LLM's first file compilation attempt
![Figure 5: Radiant Software demonstrates that the files did not compile and fails](images/ai1.png)
:::

::: {.callout-note collapse="true"}
## Figure 6: Recomplation for correction
![Figure 6: Recomplication after the LLM was prompted to correct itself. SystemVerilog code included. Still led to a failed simulation](images/ai2.png)
:::
